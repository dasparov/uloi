...
|---|---|---|---|
| 1 | **Streak + daily phrase notification** — one phrase/day from the Phrasebook (speaker audio when available), luxe-styled | Retention backbone; dead simple | S, no backend |
| 2 | **Listening quiz** — hear the Konkani (a local speaker's recording first), pick the English from 4 options | Trains the *ear*; needs zero speech recognition, so it's reliable today | S, no backend |
...
| 8 | **Duet mode** — the speaker's recording and learner's attempt side by side; shareable clip | Social + emotional hook; uses existing recorder | M |
| 9 | **Pilot leaderboard** — weekly recap scores across the 10 pilots + 5 local speakers | Friendly pressure; needs backend | M, backend |
...
- **Principles**: local businesses only · flat monthly sponsorship · clear "Sponsored" label · zero user
  data shared · revenue split toward local-speaker contributors + server costs.
...
- Local-speaker gut-check of the "Uloi" Romi spelling + starter-pack phrases (first flywheel task).
- Romi authentic spellings from local speakers replacing ICU transliteration where corrected.
